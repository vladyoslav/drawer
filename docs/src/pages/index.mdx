...
import { ComplexDrawer } from '@/entities/complex-drawer'
...
<Example>
  ...
  And you can even combine all these features together to build a non-dismissible `Drawer` that fades overlay and scales background from the second snap point, blocks interaction
  with the background and enables content scrolling only from the last snap point! Whoa! 😵‍💫
